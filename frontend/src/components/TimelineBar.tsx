...
export const TimelineBar: React.FC<TimelineBarProps> = ({ activityBlocks }) => {
  // Generate 24 hourly slots (0 - 23)
  const hours = Array.from({ length: 24 }, (_, i) => i);

  // Group activity intensity per hour
  ...
  return (
    <div className="w-full bg-slate-900 border border-slate-800 rounded-2xl p-5">
      <div className="flex items-center justify-between mb-4">
        <h4 className="text-sm font-bold text-slate-200">24-Hour Activity Heatmap</h4>
        <div className="flex flex-wrap items-center gap-3 text-xs">
          <div className="flex items-center gap-1.5">
            <span className="w-2.5 h-2.5 rounded-sm bg-sky-500" />
            <span className="text-slate-400">Work Apps</span>
          </div>
          <div className="flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-slate-400">Idle Breaks</span>
          </div>
        </div>
      </div>

      {/* Visual Bar Grid */}
      <div className="grid grid-cols-24 gap-1 h-12 bg-slate-950 p-1.5 rounded-xl border border-slate-800/80">
        {hourlyData.map((slot) => {
          let bgClass = 'bg-slate-900/60 hover:bg-slate-800';
          if (slot.category) {
          ...
          return (
            ...
            >
              {/* Tooltip */}
              <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:flex flex-col items-center bg-slate-900 border border-slate-700 text-white text-[10px] py-1 px-2 rounded shadow-xl whitespace-nowrap z-30">
                ...
                {slot.totalSeconds > 0 ? (
                  ...
                  <span className="text-slate-500">No activity logged</span>
                )}
              </div>
            </div>
          );
      ...
      {/* Hour Labels */}
      <div className="grid grid-cols-12 text-[10px] text-slate-400 mt-2 px-1">
        <span>00:00</span>
        <span>02:00</span>
        <span>04:00</span>
        <span>06:00</span>
